...
void Syrk_xx_var1( FLA_Obj, FLA_Obj, FLA_Obj );
void Syrk_xx_var2( FLA_Obj, FLA_Obj, FLA_Obj );
void Syrk_xx_var3( FLA_Obj, FLA_Obj, FLA_Obj );
void Syrk_xx_var4( FLA_Obj, FLA_Obj, FLA_Obj );
...
int main(int argc, char *argv[])
{
  ...
  for ( n=nfirst; n<= nlast; n+=ninc ){
    ...
    /* Time reference implementation (from libflame) */
      ...
      /* Compute Cref = A * A' + Cref or Cref = A' * A + Cref,
	 where C is symmetric stored in the
	 indicated triangular part of array A, by calling FLA_Syrk.  The
	 result ends up in Cef, which we will consider to be the correct
	 result. */
      FLA_Syrk( UPLO, TRANS, FLA_ONE, Aobj, FLA_ONE, Cref );
    ...
    for ( irep=0; irep<nrepeats; irep++ ){
      ...
      FLA_Syrk( UPLO, TRANS, FLA_ONE, Aobj, FLA_ONE, Cobj );
